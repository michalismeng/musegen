...
generator = loadModelAndWeights(os.path.join(note_generator_dir, 'model.json'), os.path.join(note_generator_dir, 'weights-{:02d}.hdf5'.format(generator_epoch)))
...
os.makedirs(os.path.join(output_dir, 'note-generator'), exist_ok=True)
...
for index, notes in enumerate(pieces):
    ...
    midi_stream.write('midi', fp=os.path.join(output_dir, 'note-generator', 'sample-{}.mid'.format(index)))